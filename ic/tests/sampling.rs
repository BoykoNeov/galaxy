//! Statistical validation of Plummer IC sampling. The analytic profile
//! functions must be mutually consistent, and a realization drawn from the
//! distribution function must reproduce the analytic mass profile, be properly
//! recentered, and satisfy the virial theorem (i.e. velocities are scaled
//! correctly to the potential). Expectations are independent closed forms, not
...
#[test]
fn density_central_value_and_mass_derivative_agree() {
    let p = Plummer::new(1.0, 3.0, 2.0);
    // ρ(0) = 3M / (4π a³).
    let rho0 = 3.0 * p.total_mass / (4.0 * PI * p.scale_radius.powi(3));
    assert!((p.density(0.0) - rho0).abs() < 1e-12 * rho0, "ρ(0) wrong");
    // The shell mass 4π r² ρ(r) must equal dM(<r)/dr — density and enclosed
    // mass have to describe the same model.
...
fn enclosed_mass_limits_and_half_mass_radius() {
    ...
    // r_h ≈ 1.30477 a and must enclose exactly half the mass.
    let rh = p.half_mass_radius();
    assert!(
        (rh / p.scale_radius - 1.30477).abs() < 1e-4,
        "r_h/a = {}",
        rh / p.scale_radius
    );
    assert!(
...
fn potential_and_energies_match_closed_forms() {
    ...
    let p = Plummer::new(g, m, a);
    // Φ(0) = −GM/a.
    assert!((p.potential(0.0) + g * m / a).abs() < 1e-12, "Φ(0) wrong");
    // W = −(3π/32) G M² / a; virial T = −W/2; t_dyn = √(a³/GM).
    let w = -3.0 * PI * g * m * m / (32.0 * a);
    assert!(
...
fn realized_mass_profile_matches_analytic_cdf() {
    let p = Plummer::new(1.0, 1.0, 1.0);
    let n = 40_000;
    let s = p.sample(n, 0x5EED);
    let r = sorted_radii(&s);
    // Empirical fraction within each radius vs analytic M(<r)/M.
    for &rr in &[0.5_f64, 1.0, 1.305, 2.0, 4.0] {
        let frac = r.partition_point(|&x| x <= rr) as f64 / n as f64;
        let expected = p.enclosed_mass(rr) / p.total_mass;
        // 4σ binomial band (σ = √(p(1−p)/N)) plus a small floor.
...
fn sample_is_recentered_equal_mass_and_deterministic() {
    ...
    let n = 5000;
    let s = p.sample(n, 42);
    // Recentered: COM and net momentum vanish to roundoff.
    assert!(
    ...
    // Equal masses summing to the total.
